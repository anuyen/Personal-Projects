...
let checker = str.toLowerCase().match(/[a-z]/g)

//final code
function palindrome(str) {
    let orig = str.toLowerCase().match(/[a-z\d]/g).join("")
    let rev = str.toLowerCase().match(/[a-z\d]/g).reverse().join("")
    if(orig == rev || str.match(/[a-z\d]/g) == null){
        return true;
    } else {
        return false
    }
